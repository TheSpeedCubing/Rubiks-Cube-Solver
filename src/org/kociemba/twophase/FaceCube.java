package org.kociemba.twophase;

import static org.kociemba.twophase.EnumColor.*;
import static org.kociemba.twophase.EnumCorner.URF;
import static org.kociemba.twophase.EnumEdge.UR;
import static org.kociemba.twophase.EnumFacelet.*;


class FaceCube {
    public EnumColor[] f = new EnumColor[54];

    final static EnumFacelet[][] cornerFacelet = {{U9, R1, F3}, {U7, F1, L3}, {U1, L1, B3}, {U3, B1, R3},
            {D3, F9, R7}, {D1, L9, F7}, {D7, B9, L7}, {D9, R9, B7}};

    final static EnumFacelet[][] edgeFacelet = {{U6, R2}, {U8, F2}, {U4, L2}, {U2, B2}, {D6, R8}, {D2, F8},
            {D4, L8}, {D8, B8}, {F6, R4}, {F4, L6}, {B6, L4}, {B4, R6}};


    final static EnumColor[][] cornerColor = {{U, R, F}, {U, F, L}, {U, L, B}, {U, B, R}, {D, F, R}, {D, L, F},
            {D, B, L}, {D, R, B}};


    final static EnumColor[][] edgeColor = {{U, R}, {U, F}, {U, L}, {U, B}, {D, R}, {D, F}, {D, L}, {D, B},
            {F, R}, {F, L}, {B, L}, {B, R}};

    FaceCube() {
        String s = "UUUUUUUUURRRRRRRRRFFFFFFFFFDDDDDDDDDLLLLLLLLLBBBBBBBBB";
        for (int i = 0; i < 54; i++)
            f[i] = EnumColor.valueOf(s.substring(i, i + 1));

    }


    FaceCube(String cubeString) {
        for (int i = 0; i < cubeString.length(); i++)
            f[i] = EnumColor.valueOf(cubeString.substring(i, i + 1));
    }


    String to_String() {
        String s = "";
        for (int i = 0; i < 54; i++)
            s += f[i].toString();
        return s;
    }


    CubieCube toCubieCube() {
        byte ori;
        CubieCube ccRet = new CubieCube();
        for (int i = 0; i < 8; i++)
            ccRet.cp[i] = URF;
        for (int i = 0; i < 12; i++)
            ccRet.ep[i] = UR;
        EnumColor col1, col2;
        for (EnumCorner i : EnumCorner.values()) {

            for (ori = 0; ori < 3; ori++)
                if (f[cornerFacelet[i.ordinal()][ori].ordinal()] == U || f[cornerFacelet[i.ordinal()][ori].ordinal()] == D)
                    break;
            col1 = f[cornerFacelet[i.ordinal()][(ori + 1) % 3].ordinal()];
            col2 = f[cornerFacelet[i.ordinal()][(ori + 2) % 3].ordinal()];

            for (EnumCorner j : EnumCorner.values()) {
                if (col1 == cornerColor[j.ordinal()][1] && col2 == cornerColor[j.ordinal()][2]) {

                    ccRet.cp[i.ordinal()] = j;
                    ccRet.co[i.ordinal()] = (byte) (ori % 3);
                    break;
                }
            }
        }
        for (EnumEdge i : EnumEdge.values())
            for (EnumEdge j : EnumEdge.values()) {
                if (f[edgeFacelet[i.ordinal()][0].ordinal()] == edgeColor[j.ordinal()][0]
                        && f[edgeFacelet[i.ordinal()][1].ordinal()] == edgeColor[j.ordinal()][1]) {
                    ccRet.ep[i.ordinal()] = j;
                    ccRet.eo[i.ordinal()] = 0;
                    break;
                }
                if (f[edgeFacelet[i.ordinal()][0].ordinal()] == edgeColor[j.ordinal()][1]
                        && f[edgeFacelet[i.ordinal()][1].ordinal()] == edgeColor[j.ordinal()][0]) {
                    ccRet.ep[i.ordinal()] = j;
                    ccRet.eo[i.ordinal()] = 1;
                    break;
                }
            }
        return ccRet;
    }
}
